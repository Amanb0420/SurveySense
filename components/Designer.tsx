...
function Designer({id}:{id: number}) {
    ...
    <div className='flex w-full h-full'>
        <div className='p-4 w-full' onClick={()=>
        {
            ...
            className={cn('bg-background w-full h-full m-auto rounded-xl flex flex-col flex-grow items-center justify-start flex-1 overflow-y-auto',
            droppable.isOver && "ring-2 ring-primary/20")}
            >
                {!droppable.isOver && elements.length == 0 && (<p className='text-3xl text-muted-foreground flex flex-grow items-center'>
                    Drop here
                </p>)}
...

            {
                mouseIsOver && (
                    ...
                    <div className='absolute right-0 h-full'>
                        <Button className='flex justify-center h-full border border rounded-md 
                        rounded-l-none bg-red-500' variant={"outline"} 
                        onClick={(e)=>{
                            e.stopPropagation()
                            removeElement(element.id)
                        }}>
                           <BiSolidTrash className='h-6 2-6'/> 
                        </Button>
                    </div>
